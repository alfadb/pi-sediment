...
import { EVAL_SYSTEM_PROMPT, buildEvalPrompt } from "./prompts.js";
import type { EvalResult, ResolvedModel, TargetStatus } from "./types.js";
...
function extractEvalJson(text: string): EvalResult | null {
  let body = text;
  ...
  try {
    const response = await completeSimple(
      resolved.model,
      {
        ...
        messages: [{
          role: "user",
          content: [{ type: "text", text: buildEvalPrompt({ lastAssistantMessage, gbrainColdStart }) }],
          timestamp: Date.now(),
        }],
      },
      {
